...
int resend_data_print_errors( struct pollfd *pollfds, int src, int dst ) {
	...
	// do { 
		...
		if( received == -1 ) {
			...
			sent = send( dst_fd, buf, received, 0 );
			if( sent == received )
				printf( "%ld bytes were transmitted\n", received );
			else if( sent == -1 ) {
				fprintf( stderr, "Transmission error\n" );
				return -1;
			} else {
				printf( "%ld bytes received, %ld bytes sent\n", received, sent );
			}
		}
	// } while( received != 0 );
